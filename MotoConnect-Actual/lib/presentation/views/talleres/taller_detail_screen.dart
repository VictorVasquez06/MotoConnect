...
import '../../blocs/talleres/talleres_event.dart';
import '../../blocs/talleres/talleres_state.dart';
...
class _TallerDetailScreenState extends State<TallerDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<TalleresBloc>().add(
        TalleresLoadDetail(tallerId: widget.tallerId),
      );
    });
  }

  /// Obtiene el taller seleccionado del estado
  TallerModel? _getSelectedTaller(TalleresState state) {
    if (state is TalleresLoaded) {
      return state.selectedTaller;
    }
    return null;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocBuilder<TalleresBloc, TalleresState>(
        builder: (context, state) {
          if (state is TalleresLoading || state is TalleresLoadingDetail) {
            return const Center(child: CircularProgressIndicator());
          }

          if (state is TalleresError) {
            return Center(
              child: Column(
          ...
          final taller = _getSelectedTaller(state);
          if (taller == null) {
    ...
    return Column(
      ...
      children: [
        ...
        Card(
          ...
          child: Padding(
            padding: const EdgeInsets.all(16.0),
            child: Text(taller.schedule!, style: const TextStyle(fontSize: 16)),
          ),
        ),
      ],
    );
  ...
  Widget _buildBottomActions() {
    return BlocBuilder<TalleresBloc, TalleresState>(
      builder: (context, state) {
        final taller = _getSelectedTaller(state);
  ...
  void _showAppointmentDialog(TallerModel taller) {
    showDialog(
      context: context,
      builder:
