...
class GrupoRepositoryImpl implements GrupoRepository {
  ...
  Future<List<MiembroGrupoModel>> obtenerMiembrosGrupo(String grupoId) async {
    try {
      final response = await _supabase
          .from('miembros_grupo')
          .select('*, usuarios!miembros_grupo_usuario_id_fkey(*)')
          .eq('grupo_id', grupoId)
          .order('fecha_union');
  ...
  Future<List<ParticipanteSesionModel>> obtenerParticipantes(
    ...
    if ((participantesData as List).isEmpty) return [];
  ...
  Future<ParticipanteSesionModel> aprobarParticipante({
    ...
    final participanteData = await _supabase
        .from('participantes_sesion')
        .select('*, usuarios!participantes_sesion_usuario_id_fkey(nombre, apodo, foto_perfil_url)')
        .eq('id', participanteId)
        .single();
  ...
  Future<void> verificarConexion() async {
    // Query mínimo para confirmar conectividad con Supabase
    await _supabase.from('participantes_sesion').select('id').limit(1);
  }
