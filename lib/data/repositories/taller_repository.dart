...
class TallerRepository {
  ...
  Future<List<TallerService>> getTallerServices(String tallerId) async {
    try {
      ...
      return services.map((service) => TallerService.fromString(service)).toList();
    } catch (e) {
  ...
  Future<List<TallerReview>> getTallerReviews(String tallerId) async {
    try {
      ...
      return reviewsData.map((review) => TallerReview.fromJson(review)).toList();
    } catch (e) {
  ...
  Future<List<TallerModel>> getTalleresByCategory(String category) async {
    try {
      ...
      throw Exception('Error al obtener talleres por categoría: ${e.toString()}');
    }
  }
  ...
  Future<List<TallerModel>> getTopRatedTalleres({int limit = 10}) async {
    try {
      ...
      throw Exception('Error al obtener talleres mejor calificados: ${e.toString()}');
    }
  }
