...
class PublicacionConAutor {
  ...
  final Map<String, dynamic>? eventoCompartidoData; // Datos del evento compartido
  final String? nombreOrganizadorEvento;      // Nombre del creador original del evento
...
class _ComunidadScreenState extends State<ComunidadScreen> {
  ...
  Future<void> _obtenerPublicaciones() async {
    ...
    try {
      ...
      for (var publicacionData in respuestaPublicaciones) {
        ...
        String? idEventoComp = publicacionData['referencia_evento_id'] as String?;

        final tipoPublicacion = publicacionData['tipo'] as String?;

        if (tipoPublicacion == 'ruta_compartida' && idRutaComp != null) {
          try {
            final rutaRes = await Supabase.instance.client
        ...
        } else if (tipoPublicacion == 'evento_compartido' && idEventoComp != null) {
          try {
            // Obtener todos los datos del evento
            final eventoRes = await Supabase.instance.client
                .from('eventos')
            ...
            if (organizadorIdEvento != null) {
              nombreOrganizadorEv = await _obtenerNombreUsuario(organizadorIdEvento);
            }
          } catch (e) {
            print("Error obteniendo datos del evento compartido $idEventoComp: $e");
        ...
        tempLista.add(PublicacionConAutor(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Error al cargar publicaciones: ${e.toString()}")));
  ...
  Future<void> _crearPublicacionTexto() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Error al crear la publicación: ${e.toString()}")));
      }
    } finally {
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            const Text("Aún no hay publicaciones.", style: TextStyle(fontSize: 16, color: Colors.grey)),
                              const SizedBox(height: 8),
                            Text("¡Sé el primero en compartir algo!", style: TextStyle(fontSize: 14, color: Colors.grey[600])),
                          ],
                        ),
                      )
                    : RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final nombreAutor = item.nombreAutor; // Quién compartió en comunidad
                            final nombreRutaCompartida = item.nombreRutaCompartida;
                            final idRutaCompartida = item.idRutaCompartida;
                            final eventoCompartido = item.eventoCompartidoData; // Datos del evento
                            final nombreOrganizadorEvento = item.nombreOrganizadorEvento; // Nombre del creador del evento

                            String fechaFormateadaPublicacion = 'Fecha desconocida';
                            if (publicacion['fecha'] != null) {
                              try {
                                fechaFormateadaPublicacion = DateFormat('dd MMM yy, hh:mm a', 'es_CO')
                                    .format(DateTime.parse(publicacion['fecha']).toLocal());
                              } catch (e) {
                                print("Error formateando fecha publicación: $e");
                                fechaFormateadaPublicacion = publicacion['fecha'];
                              }
                            }
                            final String tipoPublicacion = publicacion['tipo'] ?? 'texto';
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                      ...
                                      children: [
                                        ...
                                        Expanded(
                                          child: Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                              Text(nombreAutor ?? "Usuario", style: const TextStyle(fontWeight: FontWeight.bold)),
                                              Text(fechaFormateadaPublicacion, style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Colors.grey[600])),
                                            ],
                                          ),
                                        ),
                                      ],
                                    ...
                                    if (publicacion['contenido'] != null && (publicacion['contenido'] as String).isNotEmpty)
                                      Padding(
                                    ...
                                    if (tipoPublicacion == 'ruta_compartida')
                                      Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          Text("Ha compartido la ruta:", style: TextStyle(fontStyle: FontStyle.italic, color: Colors.grey[700])),
                                          Text(nombreRutaCompartida ?? "Nombre de ruta no disponible", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                          const SizedBox(height: 8),
                                          ElevatedButton.icon(
                                            ...
                                            onPressed: () {
                                              if (idRutaCompartida != null) {
                                                Navigator.pushNamed(context, '/rutas', arguments: {'ruta_id_para_cargar': idRutaCompartida});
                                              } else {
                                                ScaffoldMessenger.of(context).showSnackBar(
                                    ...
                                    if (tipoPublicacion == 'evento_compartido' && eventoCompartido != null)
                                      Container(
                                        padding: const EdgeInsets.all(12.0),
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            Row(
                                              children: [
                                                Icon(Icons.event_note_rounded, color: Theme.of(context).colorScheme.primary, size: 22),
                                                const SizedBox(width: 8),
                                                Expanded(
                                                  child: Text(
                                                    eventoCompartido['titulo'] ?? 'Evento Compartido',
                                                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 17),
                                                  ),
                                                ),
                                              ],
                                            ),
                                            if (nombreOrganizadorEvento != null && nombreOrganizadorEvento != "Usuario Anónimo" && nombreOrganizadorEvento != nombreAutor) // No mostrar si el que comparte es el mismo organizador
                                               Padding(
                                                 padding: const EdgeInsets.only(top: 2.0, bottom: 4.0),
                                                 child: Text("Organizado por: $nombreOrganizadorEvento", style: Theme.of(context).textTheme.bodySmall?.copyWith(fontStyle: FontStyle.italic)),
                                               ),
                                            const SizedBox(height: 6),
                                            if (eventoCompartido['fecha_hora'] != null)
                                              Text(
                                                "🗓️ Fecha: ${DateFormat('dd MMM yy, hh:mm a', 'es_CO').format(DateTime.parse(eventoCompartido['fecha_hora']).toLocal())}",
                                                style: Theme.of(context).textTheme.bodyMedium
                                              ),
                                            const SizedBox(height: 4),
                                            if (eventoCompartido['ubicacion'] != null && (eventoCompartido['ubicacion'] as String).isNotEmpty)
                                              Text("📍 Lugar: ${eventoCompartido['ubicacion']}", style: Theme.of(context).textTheme.bodyMedium),

                                            if (eventoCompartido['descripcion'] != null && (eventoCompartido['descripcion'] as String).isNotEmpty)
                                              Padding(
                                                padding: const EdgeInsets.only(top: 6.0),
                                                child: Text("ℹ️ ${eventoCompartido['descripcion']}", style: Theme.of(context).textTheme.bodySmall),
                                              ),
                                            const SizedBox(height: 10),
                                            if (eventoCompartido['id'] != null && eventoCompartido['titulo'] != 'Evento no disponible') // Solo mostrar si el evento es válido
                                              Align(
                                                  ...
                                                  onPressed: () {
                                                      showDialog(
                                                        ...
                                                        builder: (ctx) => AlertDialog(
                                                          title: Text(eventoCompartido['titulo'] ?? "Detalles del Evento"),
                                                          content: SingleChildScrollView(
                                                            child: ListBody(
                                                              children: <Widget>[
                                                                if (nombreOrganizadorEvento != null) Text("Organizador: $nombreOrganizadorEvento"),
                                                                const SizedBox(height: 5),
                                                                if (eventoCompartido['fecha_hora'] != null) Text("Fecha: ${DateFormat('dd MMM yy, hh:mm a', 'es_CO').format(DateTime.parse(eventoCompartido['fecha_hora']).toLocal())}"),
                                                                const SizedBox(height: 5),
                                                                if (eventoCompartido['ubicacion'] != null) Text("Lugar: ${eventoCompartido['ubicacion']}"),
                                                                const SizedBox(height: 5),
                                                                if (eventoCompartido['descripcion'] != null) Text("Descripción: ${eventoCompartido['descripcion']}"),
                                            ...
                                            else if (eventoCompartido['titulo'] == 'Evento no disponible')
                                              Text(eventoCompartido['descripcion'] ?? "Este evento ya no está disponible.", style: TextStyle(color: Colors.red.shade700, fontStyle: FontStyle.italic)),
