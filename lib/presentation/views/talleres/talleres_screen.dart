...
class _TalleresScreenState extends State<TalleresScreen> {
  ...
  Future<Map<String, dynamic>?> _mostrarDialogoTaller(
    ...
    if (modoEdicion) {
      ...
      selectedAddressString = tallerExistente['direccion'] as String?;
                  ...
                  onPressed: () {
                    if (formKeyDialog.currentState!.validate()) {
                      ...
                      Map<String, dynamic> datosDevueltos = {
                        ...
                        'direccion':
                            direccionFinal.isEmpty ? null : direccionFinal,
                        'telefono':
  ...
  Future<void> _accionCrearOEditarTaller({
    Map<String, dynamic>? tallerExistente,
  }) async {
    final Map<String, dynamic>? datosTaller = await _mostrarDialogoTaller(
      context,
      tallerExistente: tallerExistente,
    );
    ...
    if (datosTaller != null) {
      ...
      final datosParaGuardar = {
        ...
        'creado_por':
            tallerExistente == null
                ? currentUserUid
                : tallerExistente['creado_por'],
      };
      ...
      try {
        if (tallerExistente == null) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("Taller creado con éxito.")),
            );
          }
        } else {
          await Supabase.instance.client
              ...
              .eq('id', tallerExistente['id'] as String);
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("Taller actualizado con éxito.")),
            );
          }
        }
        _obtenerTalleres();
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("Error al guardar el taller: ${e.toString()}"),
            ),
          );
        }
      } finally {
  ...
  Future<void> _eliminarTaller(String tallerId, String nombreTaller) async {
    ...
    if (confirmar == true) {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Taller '$nombreTaller' eliminado.")),
          );
          _obtenerTalleres();
        }
      } catch (e) {
  ...
  Future<void> _compartirTallerEnComunidad(
    ...
    try {
      String contenidoPublicacion =
          mensajeUsuario?.isNotEmpty == true ? mensajeUsuario! : "";
              ...
              : RefreshIndicator(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final nombreTaller =
                        taller['nombre'] as String? ?? 'Nombre no disponible';
                    ...
                    final esCreador =
                        (currentUserUid != null &&
                            taller['creado_por'] == currentUserUid);
                                  ...
                                  children: [
                                    ...
                                    if (esCreador)
                                      PopupMenuButton<String>(
                                        ...
                                        onSelected: (value) {
                                          if (value == 'editar') {
                                            _accionCrearOEditarTaller(
                                              tallerExistente: taller,
                                            );
