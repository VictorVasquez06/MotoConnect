...
class _MapaCompartidoScreenState extends State<MapaCompartidoScreen>
  ...
  bool _hayParticipanteSinConexion = false;
  Timer? _timerParticipanteBanner;
  ...
  void dispose() {
    ...
    _timerParticipanteBanner?.cancel();
  ...
  Future<void> _actualizarMarcadores(
    ...
    for (final ubicacion in ubicaciones) {
      ...
      // Leer estado pre-calculado del BLoC (fuente única de verdad)
      final trackingState = context.read<MapaTrackingBloc>().state;
      final estado = trackingState.estadosPorParticipante[ubicacion.usuarioId]
          ?? EstadoConexion.activo;

      // Obtener o crear marcador con estado visual según conexión
      final icon = await _markerManager.getOrCreateMarker(
        ...
        isPaused: estado == EstadoConexion.pausado,
        isStale: estado == EstadoConexion.sinActualizacion,
        isDisconnected: estado == EstadoConexion.sinConexion,
      );
      ...
      switch (estado) {
        ...
        case EstadoConexion.sinActualizacion:
          estadoLabel = ' (Sin actualización)';
        case EstadoConexion.pausado:
          estadoLabel = ' (Pausado)';
        case EstadoConexion.activo:
          estadoLabel = '';
      }
  ...
  Future<void> _preGenerarMarcadores(
    ...
    for (var i = 0; i < participantes.length; i += maxConcurrent) {
      ...
      for (final participante in batch) {
        // Pre-generar las 4 variantes visuales: activo, pausado, stale, desconectado
        final variantes = [
          (isPaused: false, isStale: false, isDisconnected: false), // activo
          (isPaused: true, isStale: false, isDisconnected: false),  // pausado
          (isPaused: false, isStale: true, isDisconnected: false),  // stale
          (isPaused: false, isStale: false, isDisconnected: true),  // desconectado
        ];
          ...
          futures.add(
            _markerManager
                .getOrCreateMarker(
                  ...
                  isPaused: v.isPaused,
                  isStale: v.isStale,
                  isDisconnected: v.isDisconnected,
                )
                .catchError((e) {
                  debugPrint(
    ...
    return BlocListener<MapaSesionBloc, MapaSesionState>(
      listener: (context, state) {
        if (mounted && state.participantes != _participantesCache) {
          ...
          // Event-bridge: sincronizar participantes con MapaTrackingBloc
          // para que calcule estadosPorParticipante correctamente.
          context.read<MapaTrackingBloc>().add(
            MapaTrackingParticipantesSincronizados(state.participantesMap),
          );
        ...
        if (state.status == MapaSesionStatus.ready) {
          if (mounted) {
            // Siempre sincronizar para garantizar que el estado local refleje el BLoC
            if (_estaAprobado != state.estaAprobado || _esLider != state.esLider) {
              setState(() {
                _estaAprobado = state.estaAprobado;
                _esLider = state.esLider;
              });
            }
            // Iniciar tracking exactamente una vez cuando el usuario está aprobado
            if (state.estaAprobado && !_trackingIniciado) {
        ...
        children: [
          ...
          BlocConsumer<MapaTrackingBloc, MapaTrackingState>(
            listenWhen: (previous, current) =>
                previous.ubicaciones != current.ubicaciones ||
                previous.estadosPorParticipante != current.estadosPorParticipante,
            listener: (context, trackingState) {
              if (mounted) {
                _actualizarMarcadores(trackingState.ubicaciones);

                // Detectar si algún participante remoto perdió conexión (flag en BD)
                final hayAlguienSinConexion = trackingState
                    .estadosPorParticipante.entries
                    .where((e) => e.key != _miUsuarioId)
                    .any((e) => e.value == EstadoConexion.sinConexion);
                if (hayAlguienSinConexion && !_hayParticipanteSinConexion) {
                  // Transición false → true: mostrar banner + iniciar auto-dismiss 5s
                  setState(() => _hayParticipanteSinConexion = true);
                  _timerParticipanteBanner?.cancel();
                  _timerParticipanteBanner = Timer(const Duration(seconds: 5), () {
                    if (mounted) {
                      setState(() => _hayParticipanteSinConexion = false);
                    }
                  });
                } else if (!hayAlguienSinConexion && _hayParticipanteSinConexion) {
                  // Participante recuperó conexión antes de 5s: ocultar inmediatamente
                  _timerParticipanteBanner?.cancel();
                  _timerParticipanteBanner = null;
                  setState(() => _hayParticipanteSinConexion = false);
                }
              }
            },
          ...
          // Banner sutil de participante sin conexión
          _buildBannerParticipanteSinConexion(),
          ...
          BlocBuilder<MapaSesionBloc, MapaSesionState>(
            builder: (context, state) {
              ...
              if (state.status == MapaSesionStatus.esperandoAprobacion) {
                return Positioned.fill(child: _buildPantallaEspera(state));
              }
  ...
  void _mostrarSolicitudesPendientes() {
    ...
    // Capturar referencias antes de entrar al builder del dialog
    final bloc = context.read<MapaSesionBloc>();
    ...
    showDialog(
      ...
      builder: (dialogContext) => BlocProvider.value(
        value: bloc,
        child: BlocBuilder<MapaSesionBloc, MapaSesionState>(
          builder: (_, state) {
            final pendientes =
                state.participantes.where((p) => p.estaPendiente).toList();
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Builder(
          builder: (_) {
            // Leer estado pre-calculado del BLoC (fuente única de verdad)
            final estado = context.read<MapaTrackingBloc>().state
                .estadosPorParticipante[participante.usuarioId]
                ?? EstadoConexion.activo;
            switch (estado) {
              case EstadoConexion.sinConexion:
                return const Row(
  ...
  // ========================================
  // BANNER DE PARTICIPANTE SIN CONEXIÓN
  // ========================================

  Widget _buildBannerParticipanteSinConexion() {
    if (!_hayParticipanteSinConexion) return const SizedBox.shrink();

    // Banner de dispositivo propio tiene prioridad
    final trackingState = context.read<MapaTrackingBloc>().state;
    if (trackingState.conexionPerdida || trackingState.mostrarMensajeRestablecida) {
      return const SizedBox.shrink();
    }
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              const Expanded(
                child: Text(
                  'Un participante perdió conexión',
