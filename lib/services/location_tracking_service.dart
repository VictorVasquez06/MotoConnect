...
class LocationTrackingService {
  ...
  /// Timer de heartbeat para detectar pérdida de conexión estando estacionario
  Timer? _heartbeatTimer;

  /// Timestamp del último envío exitoso al servidor
  ...
  Future<void> iniciarTracking({
    ...
    try {
      ...
      // Heartbeat periódico para detectar pérdida de conexión estando estacionario
  ...
  /// Heartbeat periódico para detectar pérdida de conexión cuando el
  /// dispositivo está estacionario (sin uploads → sin fallos → sin detección).
  Future<void> _ejecutarHeartbeat() async {
    if (!_trackingActivo || _sesionActiva == null) return;

    // Si hubo un envío exitoso reciente, la conexión ya está verificada
    if (_ultimoEnvioExitoso != null &&
        DateTime.now().difference(_ultimoEnvioExitoso!).inSeconds < 25) {
      return;
    }

    try {
      await _grupoRepository.verificarConexion();
      _fallosConsecutivos = 0;
      if (!_conectado) {
        _conectado = true;
        if (!_conectadoController.isClosed) {
        ...
        debugPrint('🟢 Heartbeat: conexión restaurada');
      }
    } catch (e) {
      _fallosConsecutivos++;
      if (_fallosConsecutivos >= 3 && _conectado) {
        ...
        debugPrint(
          '🔴 Heartbeat: conexión perdida ($_fallosConsecutivos fallos consecutivos)',
        );
      }
    }
  }
